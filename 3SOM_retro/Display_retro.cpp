...
void plot_positions(std::string& flags,ccmpl::chart::Layout& display,
  ...
  display() = ccmpl::legend("['external map','context 2->1', 'context 3->1'], loc='upper right'");
  ...
  display()  += ccmpl::line("'b'",         std::bind(&CWeights::fill_line, std::ref(map1.cw2), _1));        flags += "#";
  ...
  display().xtitle  = "Position in map";
  display().ytitle  = "Weight";
  display() = ccmpl::legend("['context 3->2','context 1->2', 'external map'], loc='upper right'");
  display()   = ccmpl::view2d({0., 1.}, {0., 1.1}, ccmpl::aspect::fit, ccmpl::span::placeholder);
  display()  += ccmpl::line("'g'",         std::bind(&CWeights::fill_line, std::ref(map2.cw), _1));        flags += "#";
  display()  += ccmpl::line("'b'",         std::bind(&CWeights::fill_line, std::ref(map2.cw2), _1));        flags += "#";
  ...
  display().xtitle  = "Position in map";
  display().ytitle  = "Weight";
  display() = ccmpl::legend("['context 1->3', 'context 2->3','external map'], loc='upper right'");
  display()   = ccmpl::view2d({0., 1.}, {0., 1.1}, ccmpl::aspect::fit, ccmpl::span::placeholder);
  display()  += ccmpl::line("'g'",         std::bind(&CWeights::fill_line, std::ref(map3.cw), _1));        flags += "#";
  display()  += ccmpl::line("'b'",         std::bind(&CWeights::fill_line, std::ref(map3.cw2), _1));        flags += "#";
