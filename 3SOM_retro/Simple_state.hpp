...
struct Results{
  ...
  std::vector<Input> inputsz_closed;
...
double merge(const float& beta, const Acts& thal, const Acts& cort, const Acts& cort2, const Pos& p);
...
public:
  ...
  Acts        ca2;               // cortical matching
  CWeights    cw2;               // cortical weights

  Map         ma;               // map activities
  int mem_size  ;               //size of deques for memory

  std::deque<Pos> bmu_tab;
  std::deque<Pos> diff;
...
//  State(RANDOM_DEVICE& rd);
  ...
  State(RANDOM_DEVICE& rd)
    ...
    for(auto& c : cw2.content) c = uniform(rd);
