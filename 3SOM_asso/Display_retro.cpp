...
void fill_map(State& map1, State& map2, State& map3, std::vector<ccmpl::Point>& line){
  ...
  for(int i = 0; i< MAP_SIZE ; i++){
    double pos = i/500.;
    double val1 = map1.tw(map3.tw(map2.cw(pos)));
...
void fill_map_2(State& map1, State& map2,State& map3, std::vector<ccmpl::Point>& line){
  ...
  for(int i = 0; i< MAP_SIZE ; i++){
    double pos = i/500.;
    double val1 = map2.tw(map3.cw(map1.cw(pos)));
...
void plot_positions(std::string& flags,ccmpl::chart::Layout& display,
  ...
  display()  += ccmpl::hbar("'g'",         std::bind(fill_bar, std::ref(map2.bmu), _1));                   flags += "#";
  display()  += ccmpl::hbar("'r'",         std::bind(fill_bar, std::ref(map1.bmu), _1));                   flags += "#";
  ...
  display().title = "Reconstitued map";
  display()   = ccmpl::view2d({0.,1.1}, {0.,1.1}, ccmpl::aspect::equal, ccmpl::span::placeholder);
  display().xtitle  = "Map 1 dimension";
  display().ytitle  = "Map 2 dimension";
  display()  += ccmpl::line("'k-'",         std::bind(fill_map, std::ref(map1), std::ref(map2), std::ref(map3), _1)); flags += "#";

  display++;
  display().title = "Reconstitued map autre sens";
  display()   = ccmpl::view2d({0.,1.1}, {0.,1.1}, ccmpl::aspect::equal, ccmpl::span::placeholder);
  display().xtitle  = "Map 1 dimension";
  display().ytitle  = "Map 2 dimension";
  display()  += ccmpl::line("'k-'",         std::bind(fill_map_2, std::ref(map1), std::ref(map2),std::ref(map3), _1)); flags += "#";
