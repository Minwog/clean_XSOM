...
#define DAT_PREFIX "../experiences/2SOM_retro_2_phases/"
...
void def_sequence_retro(xsom::setup::Sequencer& seq,  State& map1,  State& map2,
  ...
  Results& res, std::string& figure_prefix, int& iterations, std::string& file_res
    ...
    seq.__def("test_loop");
      ...
      //next input
        seq.__step([&map1,&map2,&input_sampler,&res,&input_file](){
          ...
          res.inputsx.push_back(input_sampler[0]);
          res.inputsy.push_back(input_sampler[1]);
    ...
    seq.__step([&seq,&map1,&map2](){
      map1.set_beta(1.);
      map2.set_beta(1.);
      seq.msg_info("Learning with cortical closed for 150 iterations");
    });
    seq.__for(10);
    seq.__call("learn_batch"); //comme on commence depuis la pos théorique, on a le meme résultat avec le déplacement que sans.
    ...
    seq.__step([&seq,&map1,&map2](){
      map1.set_beta(BETA);
      map2.set_beta(BETA);
      seq.msg_info("Ouverture du cortical");
    });
    seq.__for(50);
  ...
  /* */ seq.__loop();
          ...
          seq.__plot_png([&flags](){return flags;}, figure_prefix );
          seq.__step([](){
...
auto display = ccmpl::layout(m.hostname, m.port, 16.0, 16.0,{"##","##","##",">."},
                            ccmpl::RGB(1., 1., 1.));
display.set_ratios({1.,1.},{1.,2.,2.,1.});
...
auto tl1 = xsom::layer<TWeight, Pos>(ALPHA,
            map1.x,
            t_match,
...
//MAP2
auto tl2 = xsom::layer<TWeight, Pos>(ALPHA,
              map2.x,
              t_match,
...
auto bmus = std::ofstream(input_filename);

def_sequence_retro<Random_Circle_2D>(std::ref(seq), std::ref(map1), std::ref(map2), std::ref(testing), std::ref(input_sampler), std::ref(flags) , std::ref(date),std::ref(inputs), std::ref(prefix) , std::ref(bmus), std::ref(results),std::ref(figure_prefix), std::ref(iterations), std::ref(file_res));
