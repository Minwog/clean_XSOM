...
void fill_dot(State_1& map1, State& map2, ccmpl::Point& dot) {
  dot = {map1.tw(map1.bmu),map2.tw(map2.bmu)};
}



void fill_input(State_1& map1, State& map2, ccmpl::Point& dot) {
  dot = {map1.x,map2.x};
}

void fill_map(State_1& map1, State& map2,std::vector<ccmpl::Point>& line){
  line.clear();
...
void plot_positions(std::string& flags,ccmpl::chart::Layout& display,
  State_1& map1, State& map2, Results& results){
  ...
  display()  += ccmpl::lines("'r--'",             std::bind(fill_err_lines,std::ref(results.inputsx), std::ref(results.inputsy),std::ref(results.bmu_w1_closed), std::ref(results.bmu_w2_closed), _1)); flags += "#";
  display()  += ccmpl::dots("color = 'k'",        std::bind(fill_anyline_2,std::ref(results.inputsx), std::ref(results.inputsy), _1)); flags += "#";
