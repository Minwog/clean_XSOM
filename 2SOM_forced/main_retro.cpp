...
#define FILENAME "../files/s_2D_shape1.txt"
...
#define DAT_PREFIX "../experiences/2SOM_forced/"
        ...
        seq.__step([&map1,&map2,&input_sampler,&res,&input_file](){
          ...
          //input_sampler.save(input_file);
              ...
              seq.__step([&map1,&map2](){
                ...
                //std::cerr<<th_bmu1<<" "<<th_bmu2<<" ";
                map1.reinit_bmu(th_bmu1);
                map2.reinit_bmu(th_bmu2);
                //std::cerr<<map1.bmu<<" "<<map2.bmu<<";";
              });
...
//Maps generation
  ...
  map1.load("../files/map1_w_2019-12-3_16-25.data");
  map2.load("../files/map2_w_2019-12-3_16-25.data");
...
Random_Square_2D input_sampler(N_INPUTS, FILENAME, N_TEST, TEST_FILENAME);
//Random_astro input_sampler(N_INPUTS, FILENAME, 400, TEST_FILENAME);
  //initiate inputs;
input_sampler();
map1.next(input_sampler.get().first);
map2.next(input_sampler.get().second);
...
auto display = ccmpl::layout(m.hostname, m.port, 10.0, 10.0,{"##","#.","##"},
                            ccmpl::RGB(1., 1., 1.));
...
auto cl1 = xsom::layer<CWeight, Pos>(params._alpha(std::ref(iteration)),
              ...
              std::bind(&Acts::learn,     std::ref(map1.ca),  _1),
	            std::bind(set_cweight, std::ref(map1.cw),  _1)
	      );
...
auto tl2 = xsom::layer<TWeight, Pos>(params._alpha(std::ref(iteration)),
  ...
  auto cl2 = xsom::layer<CWeight, Pos>(params._alpha(std::ref(iteration)),
              ...
              std::bind(&Acts::learn,     std::ref(map2.ca),  _1),
				      std::bind(set_cweight, std::ref(map2.cw),  _1)
...
def_sequence_retro<Random_Square_2D>(std::ref(seq), std::ref(map1), std::ref(map2), std::ref(testing), std::ref(input_sampler), std::ref(flags) , std::ref(date),std::ref(inputs), std::ref(prefix) , std::ref(bmus), std::ref(results),std::ref(figure_prefix), std::ref(iteration), std::ref(file_res),std::ref(params));
